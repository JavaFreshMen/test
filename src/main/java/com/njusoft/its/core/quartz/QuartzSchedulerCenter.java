package com.njusoft.its.core.quartz;
...
public class QuartzSchedulerCenter implements ApplicationContextAware,InitializingBean{
	...
	private Logger logger = LoggerFactory.getLogger(QuartzSchedulerCenter.class);
	//quartz 调度器
		...
		for(String key:beanMap.keySet()){
			...
			if(null != quartzTimerConfiguration){
				...
				if(null == jobClass || jobName.isEmpty()){
					logger.info("Job Class is not defined");
				}else{
					try {
						...
						logger.info("Start a quartz job [jobClass=".concat(jobClass.toString()).concat(",cronExpression=").concat(cronExpression).concat(",jobName=").concat(jobName).concat(",jobGroup=").concat(jobGroup)
						...
						if(null != quartzTimerBeanConfiguration){
							for(Class<?> beanName:quartzTimerBeanConfiguration.beanNames()){
								try{
									jobDataMap.put(beanName.getSimpleName(), applicationContext.getBean(beanName));
									logger.info("Inject bean to Job["+jobName+" beanName:"+beanName.getSimpleName()+"]: "+applicationContext.getBean(beanName));
								}catch (BeansException e) {
									logger.info("Inject bean failure: Cannot find bean named "+beanName);
								}
							}
						}
